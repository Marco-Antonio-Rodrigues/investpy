...
pynvesting is a Python package to retrieve data from [Investing.com](https://www.investing.com/), which provides data retrieval 
...
pynvesting allows the user to download both recent and historical data from all the financial products indexed at Investing.com. 
**It includes data from all over the world**, from countries such as United States, France, India, Spain, Russia, or Germany, 
amongst many others.

pynvesting seeks to be one of the most complete Python packages when it comes to financial data extraction to stop relying 
on public/private APIs since pynvesting is **FREE** and has **NO LIMITATIONS**. These are some of the features that currently lead 
pynvesting to be one of the most consistent packages when it comes to financial data retrieval.
...
[![Build Status](https://github.com/Marco-Antonio-Rodrigues/pynvesting/workflows/run_tests/badge.svg)](https://github.com/Marco-Antonio-Rodrigues/pynvesting/actions?query=workflow%3Arun_tests)
...
``$ pip install pynvesting``

Additionally, **if you want to use the latest pynvesting version instead of the stable one, you can install it from source** with the following command:

``$ pip install git+https://github.com/Marco-Antonio-Rodrigues/pynvesting.git@master``
...
Even though some pynvesting usage examples are presented on the [docs](https://investpy.readthedocs.io/usage.html), 
some basic functionality will be sorted out with sample Python code blocks. Additionally, more usage examples 
can be found under [examples/](https://github.com/Marco-Antonio-Rodrigues/pynvesting/tree/master/examples) directory, which 
contains a collection of Jupyter Notebooks on how to use pynvesting and handle its data.

:pushpin: __Note that `pynvesting.search_quotes` is the only function that ensures that the data is updated and aligned 1:1 with
the data provided by Investing.com!__

### :chart_with_upwards_trend: Recent/Historical Data Retrieval

pynvesting allows the user to **download both recent and historical data from any financial product indexed** 
...
df = pynvesting.get_stock_historical_data(stock='AAPL',
...
pynvesting, and also, parameter tuning, please read the docs.

### :mag: Search Live Data

**Investing.com search engine is completely integrated** with pynvesting, which means that any available 
...
search_result = pynvesting.search_quotes(text='apple', products=['stocks'],
...
Retrieved search results will be a `list` of `pynvesting.utils.search_obj.SearchObj` class instances, unless
`n_results` is set to 1, when just a single `pynvesting.utils.search_obj.SearchObj` class instance will be returned.
...
at Investing.com so that it can not be retrieved using pynvesting either, even though they are just a few, 
consider it.

As already presented previously, **historical data retrieval using pynvesting is really easy**. The piece of code 
presented below shows how to retrieve the past years of historical data from Bitcoin (BTC).

```python
import pynvesting

data = pynvesting.get_crypto_historical_data(crypto='bitcoin',
...
You can find the **complete pynvesting documentation** at [Documentation](https://investpy.readthedocs.io/).
...
enhancements, and ideas**. There is an open tab of [issues](https://github.com/Marco-Antonio-Rodrigues/pynvesting/issues) where 
...
Check the pynvesting GitHub Discussions page at [Discussions](https://github.com/Marco-Antonio-Rodrigues/pynvesting/discussions), 
...
:pushpin: __Note__. Usually I don't answer emails asking me questions about pynvesting, as we currently have the
GitHub Discussions tab, and I encourage you to use it. GitHub Discussions is the easiest way to contact me about 
pynvesting, so that I don't answer the same stuff more than once via email, as anyone can see the opened/answered
discussions.
...
```
pynvesting - Financial Data Extraction from Investing.com with Python developed by Alvaro Bartolome del Canto
```
...
Conclude that **pynvesting is not affiliated in any way to Investing.com or any dependant company**, the only 
requirement specified by Investing.com to develop this package was to "mention the source where data is 
retrieved from".

## Sobre

Este projeto é uma versão modificada do [investing.com scraper original](https://github.com/Marco-Antonio-Rodrigues/pynvesting) criado por Alvaro Bartolome.
